...
in
{
  home.packages = [
    (pkgs.writeShellScriptBin "qutebrowser-gl" ''
      ...
      ${sources.glpkgs.nixGLIntel}/bin/nixGLIntel qutebrowser $@
    '')
  ];
  ...
  home.file.qutebrowser-bookmarks = {
    enable = true;
  ...
  programs.qutebrowser = {
    ...
    keyBindings.normal = {
      ...
      ",pp" = "spawn --userscript qute-pass --dmenu-invocation ${pkgs.xprompt}/bin/xprompt --no-insert-mode --username-target secret --username-pattern \"user: (.+)\"";
      ",pP" = "spawn --userscript qute-pass --dmenu-invocation ${pkgs.xprompt}/bin/xprompt --no-insert-mode --password-only";
      ",pL" = "spawn --userscript qute-pass --dmenu-invocation ${pkgs.xprompt}/bin/xprompt --no-insert-mode --username-only --username-pattern \"user: (.+)\"";
      ",pO" = "spawn --userscript qute-pass --dmenu-invocation ${pkgs.xprompt}/bin/xprompt --no-insert-mode --otp-only";
      ",pS" = "spawn --userscript qute-pass --dmenu-invocation ${pkgs.xprompt}/bin/xprompt --no-insert-mode --password-only --password-pattern \"secret: (.+)\"";
