...
let
  ...
  xmessage = "${pkgs.xorg.xmessage}/bin/xmessage";
in
pkgs.writeText "PassPrompt.hs" ''
  module PassPrompt (passPrompt) where
  ...
  import           PassCredential

  data PromptType = Password | Login | OTP | Url | Generator | Editor | Removal
  ...
  passPrompt xpconfig = mkXPromptWithModes ([ XPT $ Pass Password xpconfig
                                            , XPT $ Pass Login xpconfig
                                            , XPT $ Pass OTP xpconfig
  ...
  otpAction :: String -> X ()
  otpAction passLabel = do
    otp <- io $ runPass ["otp", escapeQuote passLabel]
    typeString . Right $ otp
